...
#include "D:\DirectX 12 3D\Resourse\this book source code\Common\GameTimer.h"
...
// MSAA质量等级  (multile sampler anti anliasing)
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lparam);//lersult错误码
...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	//将上面没有处理的消息  转发给默认的窗口过程
	return DefWindowProc(hwnd, msg, wParam, lParam);
}



//实现一个CalculateFrameState函数，它主要是通过GameTime类中的数据计算FPS和MSPF，并显示在窗口栏上。注意：我开始运行到这里时，并没有出现fps和mspf，通过注释掉的调试模块，实时获得并显示了TotalTime();的值，最后才发现是在计算mSencondsPerCount时候用错了函数。所以这个调试模块很有用，关键它可以实时显示数值到窗口栏。
...
void Draw()
{
	...
	cmdList->ClearRenderTargetView(rtvHandle, DirectX::Colors::LightBlue, 0, nullptr);// 清除RT背景色为XXX色，并且不设置裁剪矩形
...
bool InitWindow(HINSTANCE hInstance, int nShowCmd)
{
	...
	if (!RegisterClass(&wc))
	{
		...
		//创建窗口,返回布尔值
		mhMainWnd = CreateWindow(L"MainWnd", L"DirectX12U", WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, width, hight, 0, 0, hInstance, 0);
	//窗口创建失败
	if (!mhMainWnd)
	{
...
// 初始化DirectX 12    封装    //少了-----/*开启D3D12调试层*/
bool InitDirect3D()
{
	...
	CreateRTV();
	CreateDSV();
	CreateViewPortAndScissorRect();//设置视口和裁剪矩形

	return true;
}

//对初始化窗口，初始化DirectX 12 封装，目的是判断
...
int Run()
{
	...
	while (msg.message != WM_QUIT)
	{
		...
		else//没消息
		{
			...
			//gt.Stop();//  GameTimer.cpp中不能返回 bool 值，因为这是指向绑定函数的指针只能用于调用函数，不能附加其他条件
			//所以需要封装了获取mStopped变量的函数---->bool GameTimer::MStopped()
...
void CreateDevice()
{
	...
	ThrowIfFailed(D3D12CreateDevice(nullptr,		// 此参数如果设置为nullptr，则使用主适配器
		D3D_FEATURE_LEVEL_11_0,		// 应用程序需要硬件所支持的最高功能级别
